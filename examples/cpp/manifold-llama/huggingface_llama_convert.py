...
def split_and_convert_process(saved_dir, factor, key, val):
    if (
        ...
        or key.find("mlp.down_proj.weight") != -1
    ):
...
def split_and_convert(args):
    ...
    model = LlamaForCausalLM.from_pretrained(
        args.in_file, torch_dtype=get_torch_dtype(args.weight_data_type)
    )
    ...
    np_weight_data_type = get_np_dtype(args.weight_data_type)
    ...
    def param_to_weights(param: torch.Tensor):
    ...
    for layer_id in range(num_layers):
        print(f"converting layer {layer_id}")
        ...
        qkv_weights_base_name = (
            f"model.layers.{layer_id}.attention.query_key_value.weight"
        )
        split_and_convert_process(saved_dir, factor, qkv_weights_base_name, qkv_weights)

        # attention dense
        o_weight = param_to_weights(
            model.state_dict()[f"model.layers.{layer_id}.self_attn.o_proj.weight"]
        ).T
        o_weight_base_name = f"model.layers.{layer_id}.attention.dense.weight"
        split_and_convert_process(saved_dir, factor, o_weight_base_name, o_weight)

        # MLP
        mlp_down_weight = param_to_weights(
            model.state_dict()[f"model.layers.{layer_id}.mlp.down_proj.weight"]
        ).T
        mlp_down_base_name = f"model.layers.{layer_id}.mlp.down_proj.weight"
        split_and_convert_process(
            saved_dir, factor, mlp_down_base_name, mlp_down_weight
        )

        mlp_gate_weight = param_to_weights(
            model.state_dict()[f"model.layers.{layer_id}.mlp.gate_proj.weight"]
        ).T
        mlp_gate_base_name = f"model.layers.{layer_id}.mlp.gate_proj.weight"
        split_and_convert_process(
            saved_dir, factor, mlp_gate_base_name, mlp_gate_weight
        )

        mlp_up_weight = param_to_weights(
            model.state_dict()[f"model.layers.{layer_id}.mlp.up_proj.weight"]
        ).T
        mlp_up_base_name = f"model.layers.{layer_id}.mlp.up_proj.weight"
        split_and_convert_process(saved_dir, factor, mlp_up_base_name, mlp_up_weight)

        # LayerNorm
        input_ln_weight = param_to_weights(
            model.state_dict()[f"model.layers.{layer_id}.input_layernorm.weight"]
        )
        input_ln_base_name = f"model.layers.{layer_id}.input_layernorm.weight"
        split_and_convert_process(
            saved_dir, factor, input_ln_base_name, input_ln_weight
        )
        ...
        split_and_convert_process(
            saved_dir, factor, post_attn_ln_base_name, post_attn_ln_weight
        )

        print(f"done layer {layer_id}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weight_data_type", type=str, default="fp32", choices=["fp32", "fp16", "bf16"]
    )
    parser.add_argument(
