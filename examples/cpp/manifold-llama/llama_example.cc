...
#include "src/fastertransformer/models/manifold-llama/Llama.h"
...
template<typename T>
void llama_worker(int                 tid,
                  manifold::Barrier&  barrier,
                  ...
                  const NcclParam&    pipeline_para_)
{
    ...
    check_cuda_error(cudaSetDevice(tid % device_count));
    ...
    printf("P%d-%d is running with GPU #%d.\n", rank, tid, device);
    if (tensor_para_size != world_size) {  // YIFAN: only tensor parallelism will use MPI
        if (world_size) {
    ...
    // TODO (YIFAN): fix this dirty hack!!
    NcclParam pipeline_para; // = pipeline_para_;
    pipeline_para.rank_       = tid;
    pipeline_para.world_size_ = pipeline_para_size;

    // Handle bad_words dictionary
    std::vector<int> bad_words;
    read_word_list("../examples/cpp/manifold-llama/bad_words.csv", bad_words);
    ...
    read_word_list("../examples/cpp/manifold-llama/stop_words.csv", stop_words);
    ...
    read_start_ids(request_batch_size,
                   ...
                   end_id,
                   1,
                   "../examples/cpp/manifold-llama/start_ids.csv");
...
void llama_example(const INIReader reader)
{
    ...
    int rank       = mpi::getCommWorldRank();
    int world_size = mpi::getCommWorldSize();
    ftNcclInitialize(tensor_para, pipeline_para, tensor_para_size, /* pipeline_para_size = */ 1);

    std::vector<std::thread> thds;
    manifold::Barrier        barrier(pipeline_para_size);
    for (int tid = 0; tid < pipeline_para_size; tid++) {
        thds.emplace_back(
            [&, tid = tid] { llama_worker<T>(tid, barrier, reader, random_seed, tensor_para, pipeline_para); });
    }
    for (auto& thd : thds)
        thd.join();
